...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== 'object' || typeof (body as { idToken?: unknown }).idToken !== 'string') {
    return mobileJson(request, { error: 'Invalid request.' }, { status: 400 });
  }
  try {
    const result = await new MobileAuthService().googleLogin(
    ...
    return mobileJson(request, { error: 'Google sign-in failed.' }, { status: 401 });
  }
}

...
export default async function CatalogPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-neutral-200 pt-6 sm:pt-10"><div className="mx-auto max-w-6xl px-4 pb-6 sm:px-6"><p className="text-xs font-bold uppercase text-[#E02424]">PDF Catalog</p><h1 className="mt-2 text-2xl font-black uppercase text-neutral-950 sm:text-4xl">{catalog.title}</h1>{catalog.description && <p className="mt-3 max-w-3xl text-sm text-neutral-600">{catalog.description}</p>}</div><CatalogWebView catalog={catalog} /></main>;
}

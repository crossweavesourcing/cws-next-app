...
export function CatalogManager({ locked }: { locked: LockedAssociation }) {
  ...
  const [editingCatalog, setEditingCatalog] = useState<SerializedCatalogDocument | null>(null);
  ...
  function edit(catalog: SerializedCatalogDocument) { setEditingCatalog(catalog); }
  ...
  return <section className="mt-8 border-t border-white/10 pt-7 text-white">
    ...
    {editingCatalog && (
      <EditCatalogModal
        ...
        onSuccess={() => { setEditingCatalog(null); void refresh(); }}
      />
    )}
  </section>;
}

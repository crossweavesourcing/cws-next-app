...
const editable: Record<string, Omit<SectionDefinition, keyof Pick<CmsSection, 'id' | 'label' | 'route' | 'summary' | 'paused' | 'status' | 'lastEdited' | 'pageKey'>>> = {
  ...
  'home-contact': {
    visibilityEditable: true,
    fields: [text('eyebrow', 'Eyebrow'), area('introduction', 'Introduction'), text('panelLabel', 'Panel label'), text('panelHeading', 'Panel heading'), area('panelBody', 'Panel body'), text('email', 'Email'), area('bangladeshAddress', 'Bangladesh address'), area('usaAddress', 'USA address'), text('formHeading', 'Form heading'), text('submitLabel', 'Submit button label')],
    mediaSlots: [],
    defaultContent: { eyebrow: 'Direct Sourcing Channels', introduction: 'Partner directly with our executive leadership to establish reliable production, quality assurance, and seamless apparel supply chains.', panelLabel: 'Contact Information', panelHeading: "Let's build your next sourcing plan.", panelBody: 'Send production details, sampling needs, or buying requirements. Our team will review the request and connect with you directly.', email: 'info@crossweavesourcing.com', bangladeshAddress: 'Bashundhara R/A, Road No. 3, Lane No. 3, House No. 1339/A, Ward No. 24, Chittagong, Bangladesh', usaAddress: 'PO Box: 41, 26 S White Horse Pike, Somerdale, NJ 08083, USA', formHeading: 'Send Us a Message', submitLabel: 'Send Request' },
  },
  'products-hero': {
    visibilityEditable: true,
